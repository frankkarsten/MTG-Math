...
def run_one_sim():	
	...
	for handsize in ['free', 7, 6, 5, 4]:
		...
		if not keephand:
			...
			if handsize == 'free' and deck_size == 99:
				#Do we keep?
				if (total_lands(hand) >= 3 and total_lands(hand) <= 5):
					keephand = True
	...
	if deck_size == 99:
		#Draw a card on turn 1 in Commander
		card_drawn = library.pop(0)
		hand[card_drawn] += 1
...
for deck_size in [40, 60, 80, 99]:
	...
	for turn_allowed in range(2, 8):
		good_lands_range = range(1, min(5, turn_allowed +1))
		for good_lands_needed in good_lands_range:
			...
			printing("--One row per number of colored sources")
